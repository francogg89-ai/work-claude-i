...
NEGATIVOS_EXIGIDOS = ["N%d" % i for i in range(1, 26)]
...
def satisface(cmd, vinculaciones):
    """Nombres de las vinculaciones que esta resolucion satisface. Puede ser mas de una."""
    return [n for n, repo, sha in vinculaciones if cmd == mod_sondas.comando(repo, sha)]


def cobertura_nominal(procesos, vinculaciones):
    """Regla de medicion: se mide sobre el conjunto nominal, nunca sobre sus valores.

    Devuelve el mapeo resolucion -> vinculaciones satisfechas, los nombres cubiertos, los que
    quedan sin cubrir y las invocaciones que no satisfacen ninguna vinculacion.
    """
    mapeo = []
    cubiertas = set()
    no_sondas = []
    for i, cmd in enumerate(procesos):
        nombres = satisface(cmd, vinculaciones)
        mapeo.append((i, cmd, nombres))
        if nombres:
            cubiertas.update(nombres)
        else:
            no_sondas.append(cmd)
    sin_cubrir = [n for n, _, _ in vinculaciones if n not in cubiertas]
    return {"mapeo": mapeo, "cubiertas": cubiertas, "sin_cubrir": sin_cubrir,
            "no_sondas": no_sondas}


def cobertura_por_valor(procesos, vinculaciones):
    """Medicion defectuosa, conservada solo como contra-insumo de N25.

    Colapsa dos vinculaciones distintas que comparten repositorio y SHA. No se usa para
    evaluar la corrida: existe para demostrar que la regla de medicion discrimina.
    """
    esperadas = {(repo, sha) for _, repo, sha in vinculaciones}
...
def cuerpo(args, log, fallos, controles, fallar_en, x0):
    ...
    log("E19 TRAZA EXTERNA DE X0 (interceptada), COBERTURA NOMINAL")
    tr = x0["traza"]
    cob = cobertura_nominal(tr.procesos, ids)
    log("  mapeo resolucion -> vinculaciones satisfechas:")
    for i, cmd, nombres in cob["mapeo"]:
        log("    #%d  %s" % (i + 1, " ".join(cmd)))
        log("        satisface: %s" % (", ".join(nombres) if nombres else "NINGUNA"))
    log("  vinculaciones nominales declaradas: %d" % len(ids))
    log("  cubiertas: %s" % sorted(cob["cubiertas"]))
    log("  sin cubrir: %s" % (cob["sin_cubrir"] or "ninguna"))
    log("  invocaciones que no son sonda: %d" % len(cob["no_sondas"]))
    log("  aperturas de archivo: %d" % len(tr.archivos))
    ok_e19 = (not cob["sin_cubrir"] and not cob["no_sondas"] and not tr.archivos)
    log("  E19 = %s" % ok_e19)
    if not ok_e19:
    ...
    no_sondas22 = cobertura_nominal(tr22.procesos, ids)["no_sondas"]
    ...
    sin_extra23 = (not cobertura_nominal(tr23.procesos, ids)["no_sondas"]
                   and not tr23.archivos)
    ...
    v25 = mod_sinteticos.N25_VINCULACIONES
    cob25 = cobertura_nominal([mod_sondas.comando(r, sh) for _, r, sh in v25], v25)
    nominal25 = len(cob25["cubiertas"])
    valor25 = cobertura_por_valor(None, v25)
    d25 = nominal25 == len(v25) and valor25 < len(v25)
    log("  N25 dos vinculaciones con el mismo repo+sha -> nominal=%d valor=%d declaradas=%d"
        % (nominal25, valor25, len(v25)))
    log("      cubiertas nominalmente: %s" % sorted(cob25["cubiertas"]))
    log("      la medicion por valor las colapsa; la nominal no. discrimina = %s" % d25)
    if not d25:
        fallos.append(("F7", "N25", "la regla de medicion no discrimina el colapso por valor"))
    ...
    presentes |= {"N%d" % i for i in range(12, 26)}
